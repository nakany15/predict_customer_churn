...
class TestEncoderHelper(object):
	...
	def encoder_fixt(self):
		df = pytest.df
		df_out = cls.encoder_helper(df, ['Income_Category','Education_Level'],'Churn')
		return df_out
	...
	def test_value_counts(self, encoder_fixt):
		try:
			assert len(encoder_fixt['Income_Category_Churn'].value_counts()) == len(encoder_fixt['Income_Category'].value_counts())
			assert len(encoder_fixt['Education_Level_Churn'].value_counts()) == len(encoder_fixt['Education_Level'].value_counts())
		except AssertionError as err:
			logging.error("number of unique values in encoded variables must be equel to original variables")
			raise err
class TestperformFeatureEngineering(object):
	'''
	test perform_feature_engineering
	'''
	@pytest.fixture(params=[['Gender'], ['Education_Level', 'Marital_Status'], ['Income_Category', 'Card_Category', 'Education_Level']])
	...
	def test_column_length(self, encoding_category_fixt):
		...
		try:
			...
			logging.error("encoder_helper didn't produced expected number of columns")
			raise err		
	...
	def test_record_not_changes(self,encoding_category_fixt):
		...
		try:
			...
			logging.error("total number of train and test must be equal to input DataFrame")
			raise err
